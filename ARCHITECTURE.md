...
Because memories are embedded with a type prefix but queries arrive without one, a bare query like "authentication strategy" has lower similarity to a stored `"webapp decision auth-strategy ..."` embedding than it would with the type prefix included.

Type-prefix fan-out closes this gap. When `QUERY_EXPANSION_FANOUT` is enabled (default), `find_similar()` embeds the query with each of the 7 memory types prepended (`"{project} fact {query}"`, `"{project} decision {query}"`, etc.), then scores each memory against all 8 vectors (7 prefixed + 1 original) and keeps the maximum similarity per memory.

This is ~7× more dot products per search but no additional model inference — the query vectors are all generated by the already-loaded model, and the memory vectors are fetched once. On a DB with 1000 memories, the overhead is ~10ms.

Benchmarked against ground truth:
- **Easy benchmark** (clean clusters): MRR 0.969 �� 1.000
- **Hard benchmark** (overlapping topics, temporal spread, distractors): overall MRR 0.875 → 0.881

Three other expansion strategies were tested (corpus-aware PRF, nearest-neighbor blending, combined) but none improved on fan-out alone. Corpus PRF actively hurt by injecting misleading FTS terms. Results are preserved in `tests/test_query_expansion.py`.
...
| `QUERY_EXPANSION_FANOUT` | True | Type-prefix fan-out in find_similar() |
...
| `test_e2e_pipeline.py` | 14 | Full round-trip through all 5 layers + gotcha — multi-turn prompt→stop→prompt flow, enforcement decisions, session isolation, metrics |
| `test_install_validation.py` | 21 | Installation validation — DB schema/migration/triggers, template files, settings.json merge/removal, script syntax, health check, config env overrides |
| `test_retrieval_benchmark.py` | 17 | Latency regression — FTS5/vector/RRF at 100/500/1000 memories, dedup throughput, sub-linear scaling guards |
| `test_retrieval_quality.py` | 13 | Retrieval quality (easy) — ground-truth P/R/MRR for semantic, FTS5, hybrid RRF; 5 clean clusters, 16 queries |
| `test_retrieval_quality_hard.py` | 12 | Retrieval quality (hard) — overlapping clusters, temporal spread, cross-project, distractors, graded difficulty tiers |
...
- **Tier 2 (file context)** — the PreToolUse hook injects per-file structural context on Read/Edit, deduped once per file. The graph is resolved from the **accessed file's own location** (walking up from the file, not cwd), so structural context still surfaces when cwd is a *parent* directory (e.g. `~/Projects`) and the edited file lives in a subrepo whose graph the fleet already built. It also recovers file paths from `Bash` commands (`cat`/`sed`/`head` + the `cch-edit.py`/`cch-write.py` helpers), so it still fires when file access is routed through Bash.
...
**Branch-switch freshness.** First-contact build + hourly sweep would leave the graph stale after a *mid-session* `git checkout` / `pull` / `rebase` until the next sweep or session. To close that gap, the prompt hook records the repo's HEAD in a per-cwd `hook_state` sentinel and, on every prompt, compares it (`repo_discovery.kick_graph_update_if_head_changed`); when HEAD has moved it kicks a background `crg update`. HEAD movement (a different checked-out commit) is the right freshness key — two branches at the same commit produce an identical tree, so no refresh is needed. This rides the same portable, no-native-git-hooks path as the rest of the fleet (so it works under git-ai and other git proxies); plain uncommitted edits remain the cron sweep's job.
...
**Provenance.** `config.GENERATION_PROMPT_VERSION` (`genA-v4`) is stamped into `memories.source_ref` at insert time by the Stop hook, so the downstream usefulness of a memory (its `memory_deliveries` grades and engagement) is attributable to the generation-prompt version that produced it. `storage.insert_memories` resolves source_ref per entry with the precedence `entry["source_ref"]` > the `source_ref` call parameter > `NULL` — so the analyser and ingest set their own provenance (`analyser-session-arc`, review-writeback) and are unaffected, while plain agent writes get the version stamp. The `genA-v1 → genA-v3` bump records the addition of the **dual-altitude transferability** lever to the live generation rules (`.claude/rules/memory-system.md`): capture the generalised, cross-project form of a lesson anchored by the specific instance, so usefulness of those memories is attributable to that change. The subsequent `genA-v3 → genA-v4` bump promoted **anticipated-question keyword seeding** (2–3 question-form phrasings per entry) into the base rules after it won as arm B of the live A/B (`genB-v1`).

**Live per-prompt A/B (`config.AB_TEST_ENABLED`, on).** The real, data-gathering experiment runs in production: the prompt hook randomly assigns each user prompt to **arm A** (control = the current live generation rules) or **arm B** (control + exactly one speculative variable, `config.AB_B_INSTRUCTION` — currently write-side meta suppression, which forbids self-referential memory-bookkeeping entries at write time), records the arm in `hook_state`, and injects arm-B's extra instruction for that turn (post-cache `additionalContext`, so the prompt cache is undisturbed). The Stop hook reads the turn's arm and stamps that turn's memories with the arm's version in `source_ref` (`config.AB_ARM_VERSIONS`: A=`genA-v4`, B=`genB-v2`); subagents are excluded. Because every delivery already logs engagement + agent grades keyed to `memory_id`, the arms are compared with no extra capture via `query.py --delivery-stats` (outcome grouped by generation version and reranker). Per-prompt randomisation (the smallest independent unit) accrues data fastest and cancels session-level confounds; the dedup-collision caveat (a near-duplicate B write merges into an existing row) is a manageable analysis nuance, not an attribution loss.

**Reading `--delivery-stats`.** The report groups every logged delivery three ways — by generation version (the write-side A/B arms), by reranker model, and by **injection layer** — reporting `n`, a `decided` count, `eng%(dec)`, mean `avgScore`, and agent `rg` grade per group. The cardinal rule: **engaged% is computed over *decided* deliveries, never over `n`.** A delivery is "decided" only once `apply_engagement` has scored it, and that runs *only at a main-session Stop* — so deliveries from read-only / one-shot `claude -p` sessions (the calibration analyser, its cron, scheduled tasks) are never scored, because the Stop hook `sys.exit(0)`s in read-only mode before reaching `apply_engagement`. Dividing engaged by `n` would therefore conflate "measured, not engaged" with "never measured" and silently understate every layer whose sessions are mostly non-interactive.

This is exactly why the raw per-layer counts are a **coverage** story, not a usefulness story. `first-prompt` (Layer 1) fires in *every* session — including thousands of read-only analyser sessions — so its coverage is very low (historically ~2% of its deliveries decided); `per-prompt` (Layer 1.5) fires *only mid-conversation*, which happens only in genuine interactive sessions that do run a Stop, so its coverage is ~100%. The earlier reading that first-prompt "engages near-zero" was an artifact of dividing by `n`: restricted to *decided* deliveries, first-prompt engages ~36% (higher in recent windows), in line with the other push layers. So always read `eng%(dec)` next to the `decided` count — a low-coverage row is weak evidence about usefulness either way, and the largest confound in the table is session type (interactive vs read-only), not layer relevance. Two fixes reduce this: v0.16.3 stopped logging deliveries in read-only sessions, and v0.16.4 has cairn's internal analysis passes (analyser / audit / offline A/B / relevance labeller) set `CAIRN_NO_INJECT=1` to skip injection entirely — those passes were ~98% of first-prompt sessions, so first-prompt volume drops sharply and coverage rises going forward. Injection is gated on `CAIRN_NO_INJECT`, not on `CAIRN_MODE=read-only`, so scheduled tasks (read-only, but recall-wanting) are unaffected.
