...
**Reading `--delivery-stats`.** The report groups every logged delivery three ways — by generation version (the write-side A/B arms), by reranker model, and by **injection layer** — reporting `n`, engagement rate, and mean agent `rg` grade per group. Two facts govern interpretation. First, **label coverage is sparse**: only deliveries whose turn produced a scorable response are engagement-scored (typically a few percent of all deliveries) and fewer still are agent-graded, so small-`n` version/arm rows are underpowered — a double-digit engagement gap over `n` in the tens is *directional, not conclusive*, and the write-side A/B in particular stays label-starved until volume accrues. Second, the **layer breakdown is the higher-powered, more actionable signal**: `first-prompt` (Layer 1) is by far the largest layer by volume yet engages near-zero — it pays the cold-start insurance premium and is rarely the memory actually used; `per-prompt` (Layer 1.5) is the workhorse, engaging on a majority of its deliveries because it matches the live prompt; `project-bootstrap` fires rarely and engages seldom but carries the highest `rg` grades when it does (high-value, low-trigger). That volume-vs-engagement disparity across layers — not the chronically underpowered arm comparison — is where retrieval-cost tuning has the most leverage, with the `first-prompt` layer the prime candidate for tighter selection or reduced volume.
