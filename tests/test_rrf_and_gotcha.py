...
def test_rrf_fts_only_results_have_proper_scores():
...
@pytest.mark.behavioural
def test_rrf_dual_match_boosted_above_single():
...
@pytest.mark.behavioural
def test_rrf_fts_exact_match_surfaces():
...
@pytest.mark.edge
def test_rrf_same_session_excluded():
...
@pytest.mark.behavioural
def test_correction_gets_associated_files():
...
#TAG: [A009]
# Verifies: non-correction memory type does not get associated_files populated even with transcript
@pytest.mark.behavioural
def test_non_correction_skips_file_association():
    """Non-correction memories should not get associated_files even with a transcript."""
    db_path, conn = fresh_db()
    conn.execute("INSERT INTO sessions (session_id, project) VALUES ('s1', 'proj')")
    conn.commit()
    ...
    assert row[0] is None  # No associated files for non-corrections
    conn.close()
...
#TAG: [A00A]
# Verifies: find_corrections_for_file matches a correction by exact file path
@pytest.mark.behavioural
def test_gotcha_finds_corrections_by_exact_path():
    """find_corrections_for_file should match on exact file path."""
    import pretool_hook
    db_path, conn = fresh_db()
    conn.execute(
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("/project/hooks/retrieval.py")
...
#TAG: [A00B]
# Verifies: find_corrections_for_file falls back to basename match when full paths differ
@pytest.mark.behavioural
def test_gotcha_finds_corrections_by_basename():
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("/new/path/config.py")
...
#TAG: [A00C]
# Verifies: archived corrections are excluded from gotcha injection results
@pytest.mark.edge
def test_gotcha_skips_archived_corrections():
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("/project/foo.py")
...
#TAG: [A00D]
# Verifies: querying for an unrelated file returns empty list with no false positives
@pytest.mark.edge
def test_gotcha_no_match_returns_empty():
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("/project/unrelated.py")
...
#TAG: [A00E]
# Verifies: result count is capped at MAX_GOTCHA_INJECTIONS when many corrections match
@pytest.mark.behavioural
def test_gotcha_respects_max_injections():
    """Should cap at MAX_GOTCHA_INJECTIONS corrections per file."""
    import pretool_hook
    db_path, conn = fresh_db()
    for i in range(10):
        conn.execute(
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("/project/buggy.py")

    assert len(matches) == pretool_hook.MAX_GOTCHA_INJECTIONS
    conn.close()
...
@pytest.mark.behavioural
def test_gotcha_hook_outputs_additional_context():
    ...
    # CAIRN GOTCHA header must appear exactly once
    assert ctx.count("CAIRN GOTCHA") == 1
    # The specific correction topic must appear exactly once
    assert ctx.count("import-bug") == 1
    conn.close()
...
# find_corrections_for_file error + adversarial
# ============================================================

#TAG: [A013]
# Verifies: malformed JSON lines in transcript are skipped and valid tool calls still collected
@pytest.mark.error
def test_extract_files_skips_malformed_json_lines():
...
@pytest.mark.adversarial
def test_extract_files_multiedit_captures_all_paths():
...
#TAG: [A015]
# Verifies: empty file_path argument to find_corrections_for_file returns empty list immediately
@pytest.mark.error
def test_gotcha_empty_file_path_returns_empty():
    """find_corrections_for_file with empty string must return [] without querying the DB."""
    import pretool_hook
    db_path, conn = fresh_db()
    conn.execute(
        "INSERT INTO memories (type, topic, content, associated_files, confidence) VALUES (?,?,?,?,?)",
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("")
...
#TAG: [A016]
# Verifies: NULL associated_files (not malformed JSON) is skipped without crashing
@pytest.mark.adversarial
def test_gotcha_null_associated_files_skipped():
    ...
    with patch.object(hook_helpers, 'DB_PATH', db_path):
        matches = pretool_hook.find_corrections_for_file("/project/target.py")
